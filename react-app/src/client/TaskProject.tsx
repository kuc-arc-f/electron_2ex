      ...
      <div className="container mx-auto px-4 py-4">
        <div className="items-center justify-between p-4 bg-white rounded-lg shadow">
          ...
          <div className="col-md-6 text-end">
            <Link to={`/task_project/create`} className="btn btn-primary">Create
            </Link>
          </div>
          <hr className="my-1" />
        ...
        {data.items.map((item, index) => (
          ...
          className="flex items-center justify-between py-4 px-4 my-4 bg-white rounded-lg shadow">
            ...
            <div className="flex-1 text-end pt-3">
              <Link to={`/task_project_show?id=${item.id}`}
              className="btn btn-sm btn-outline-blue"
              >Show</Link>
              {/*
              <a className="btn btn-sm btn-outline-blue"
              href={`/task_project/edit?id=${item.id}`}>Edit
              </a>
              <a 
                className="btn btn-sm btn-outline-blue">Gantt
              </a>
              */}
            </div>            
          </li>
